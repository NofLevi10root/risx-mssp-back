...
async function active_velocirapto_artifact(req, res, next) {
  ...
  try {
    ...
    for (const artifact of checkedArtifacts) {
      ...
      const request_file_Path_and_Name =
        await make_JSON_Artifact_to_velociraptor(artifact_id);
    ...
    console.error("Error in active_velocirapto_artifact :", err);
  }
}
...
async function active_modules(req, res, next) {
  try {
    ...
    console.error("Error in active_modules :", err);
  }
}
